...
from backend.app.services.asset_store import STORAGE_ROOT, AssetStore
from backend.app.data_access.operations.generated_image_operations import (
    ensure_generated_image_store,
    find_any_generated_image,
    find_owner_generated_image,
    insert_generated_image,
    list_recent_generated_images as list_recent_generated_image_records,
    read_generated_image,
)

logger = logging.getLogger("huit_chatbot.image_service")
...
def compute_image_canonical_hash(prompt: str, style: str, width: int, height: int, backend: str) -> str:
    """Tính canonical hash chuẩn hóa cho yêu cầu sinh ảnh."""
    p_norm = re.sub(r"\s+", " ", prompt.strip().lower())
    payload = f"img:{p_norm}:{style.lower().strip()}:{width}:{height}:{backend.lower().strip()}"
    return hashlib.sha256(payload.encode("utf-8")).hexdigest()


def generate_flux_image(req, seed: Optional[int] = None):
    ...
    if seed is None:
        if getattr(req, "regenerate", False):
            seed = random.randint(1000, 9999999)
        else:
            seed = int(hashlib.md5(f"{req.prompt}:{style}:{req.width}:{req.height}".encode("utf-8")).hexdigest()[:8], 16) % 9000000 + 1000000
...
def create_image(req: ImageRequest, owner_id: Optional[str] = None):
    if not req.prompt.strip():
        raise ValueError("Empty prompt")
    # Kiểm tra database kết nối trước khi sinh ảnh
    ensure_generated_image_store()

    style = getattr(req, "style", "photorealistic") or "photorealistic"
    backend = getattr(req, "backend", "flux") or "flux"
    c_hash = compute_image_canonical_hash(req.prompt, style, req.width, req.height, backend)
    clean_owner = owner_id if (owner_id and owner_id != "anonymous") else None
    access_scope = "private" if clean_owner else "public"

    # 1. DEDUPLICATION: Tái sử dụng ảnh vật lý và bản ghi logic nếu không yêu cầu regenerate
    if not getattr(req, "regenerate", False):
        try:
            # 1.1 Kiểm tra xem chính user này đã có logical record cho c_hash chưa
            user_existing = find_owner_generated_image(c_hash, clean_owner)
            if user_existing:
                img_id = user_existing.get("image_id") or user_existing.get("_id")
                return {
                    "image_id": img_id,
                    "id": img_id,
                    "image_url": f"/api/images/{img_id}/file" if user_existing.get("backend") == "flux" else f"/api/images/{img_id}/svg",
                    "thumbnail_url": f"/api/images/{img_id}/thumbnail",
                    "svg_url": f"/api/images/{img_id}/svg",
                    "json_url": f"/api/images/{img_id}",
                    "width": user_existing.get("width", req.width),
                    "height": user_existing.get("height", req.height),
                    "model": user_existing.get("model", "FLUX.1-schnell"),
                    "style": user_existing.get("style", style),
                    "byte_size": user_existing.get("byte_size", user_existing.get("image_bytes", 0)),
                    "checksum": user_existing.get("content_hash") or c_hash,
                    "request_fingerprint": user_existing.get("request_fingerprint") or c_hash,
                    "billing": "free",
                    "cached": True,
                    "access_scope": user_existing.get("access_scope", access_scope),
                    "owner_id": clean_owner
                }

            # 1.2 Nếu user này chưa có, nhưng nội dung ảnh đã tồn tại trên hệ thống (từ user khác):
            # Tái sử dụng physical blob trong assets, tạo logical record riêng biệt cho user hiện tại!
            existing_any = find_any_generated_image(c_hash)
            if existing_any:
                existing_blob_id = existing_any.get("blob_id") or existing_any.get("image_id")
                storage_key = existing_any.get("storage_key")
                thumbnail_key = existing_any.get("thumbnail_key") or storage_key
                byte_size = existing_any.get("byte_size", existing_any.get("image_bytes", 0))

                # Tăng reference count cho physical blob
                user_b_img_id = secrets.token_hex(24)
                new_logical_doc = {
                    "_id": user_b_img_id,
                    "schema_version": 1,
                    "image_id": user_b_img_id,
                    "owner_id": clean_owner,
                    "access_scope": access_scope,
                    "blob_id": existing_blob_id,
                    "request_fingerprint": c_hash,
                    "canonical_content_hash": c_hash,
                    "content_hash": existing_any.get("content_hash") or c_hash,
                    "backend": existing_any.get("backend", "flux"),
                    "prompt": req.prompt,
                    "width": existing_any.get("width", req.width),
                    "height": existing_any.get("height", req.height),
                    "style": existing_any.get("style", style),
                    "model": existing_any.get("model", "FLUX.1-schnell"),
                ...
                inserted_new_logical = False
                try:
                    insert_generated_image(
                        new_logical_doc,
                        principal_id=clean_owner or "anonymous",
                    )
                    inserted_new_logical = True
                except Exception as e:
                    # Nếu có race condition giữa 2 request đồng thời của cùng một user
                    from pymongo.errors import DuplicateKeyError
                    if isinstance(e, DuplicateKeyError):
                        existing_dup = find_owner_generated_image(c_hash, clean_owner)
                        if existing_dup:
                            user_b_img_id = existing_dup.get("image_id") or existing_dup.get("_id")
                        else:
                            raise e
                    else:
                        raise e

                if inserted_new_logical and existing_blob_id:
                    AssetStore.touch_asset(existing_blob_id)

                return {
                    "image_id": user_b_img_id,
                    "id": user_b_img_id,
                    "image_url": f"/api/images/{user_b_img_id}/file" if existing_any.get("backend") == "flux" else f"/api/images/{user_b_img_id}/svg",
                    "thumbnail_url": f"/api/images/{user_b_img_id}/thumbnail",
                    "svg_url": f"/api/images/{user_b_img_id}/svg",
                    "json_url": f"/api/images/{user_b_img_id}",
                    "width": existing_any.get("width", req.width),
                    "height": existing_any.get("height", req.height),
                    "model": existing_any.get("model", "FLUX.1-schnell"),
                    "style": existing_any.get("style", style),
                    "byte_size": byte_size,
                    "checksum": existing_any.get("content_hash") or c_hash,
                    "request_fingerprint": c_hash,
                    "billing": "free",
                    "cached": True,
                    "access_scope": access_scope,
            ...
            logger.error("Không thể kiểm tra dedup ảnh: %s", type(e).__name__)
            raise RuntimeError("Không thể xác minh trạng thái dedup ảnh; đã dừng trước khi gọi nhà cung cấp") from e

    # 2. Xử lý seed và generation key khi regenerate=True
    is_regen = getattr(req, "regenerate", False)
    if is_regen:
        seed = random.randint(1000, 9999999)
        generation_key = hashlib.sha256(f"{c_hash}:{seed}:{datetime.now(timezone.utc).isoformat()}:{secrets.token_hex(4)}".encode()).hexdigest()
        request_fingerprint_val = generation_key
    else:
        seed = int(hashlib.md5(f"{req.prompt}:{style}:{req.width}:{req.height}".encode("utf-8")).hexdigest()[:8], 16) % 9000000 + 1000000
        generation_key = None
        request_fingerprint_val = c_hash
    canonical_hash_val = c_hash

    storage = get_storage_adapter()
    blob_id = f"blob_img_{secrets.token_hex(16)}"
    image_id = secrets.token_hex(24)

    # 3. SINH ẢNH MỚI
    if backend == "flux":
        image_bytes, ctype = generate_flux_image(req, seed=seed)
        physical_hash = hashlib.sha256(image_bytes).hexdigest()

        # Lưu file gốc qua StorageAdapter (không bao giờ nhét binary lớn vào MongoDB)
        safe_filename = f"image_{image_id}.jpg"
        storage.put(safe_filename, image_bytes, media_type=ctype)

        # Sinh WebP thumbnail nhẹ (10-40 KB, max 256px)
        thumb_name = f"thumb_{image_id}.webp"
        thumb_bytes = b""
        try:
            ...
            storage.put(thumb_name, thumb_bytes, media_type="image/webp")
        except Exception:
            thumb_name = safe_filename

        try:
            # 3.1 Lưu Physical Blob vào collection 'assets' (không chứa owner_id)
            AssetStore.save_asset(
                asset_id=blob_id,
                media_type=ctype,
                content_hash=physical_hash,
                storage_key=safe_filename,
                preview_key=thumb_name,
            ...
            # 3.2 Lưu Logical Record vào collection 'generated_images' (có owner_id, blob_id)
            doc = {
                "_id": image_id,
                "schema_version": 1,
                "image_id": image_id,
                ...
                "content_type": ctype,
                "storage_key": safe_filename,
                "thumbnail_key": thumb_name,
                "request_fingerprint": request_fingerprint_val,
                "content_hash": physical_hash,
                "canonical_content_hash": canonical_hash_val,
                "generation_key": generation_key,
                "byte_size": len(image_bytes),
                "thumb_size": len(thumb_bytes),
                "created_at": datetime.now(timezone.utc)
            }
            insert_generated_image(doc, principal_id=clean_owner or "anonymous")
        except Exception as db_err:
            # Compensation: Xóa file đã lưu trên storage nếu ghi database thất bại (chống orphan files)
            try:
                storage.delete(safe_filename)
                if thumb_name != safe_filename:
                    storage.delete(thumb_name)
            except Exception:
                pass
            AssetStore.delete_asset_record(blob_id)
            raise db_err
    ...
    svg_xml = render_svg({"scene": scene.model_dump(), "width": req.width, "height": req.height})
    svg_bytes = svg_xml.encode("utf-8")
    physical_hash = hashlib.sha256(svg_bytes).hexdigest()
    safe_svg_filename = f"svg_{image_id}.svg"
    storage.put(safe_svg_filename, svg_bytes, media_type="image/svg+xml")
    ...
    try:
        AssetStore.save_asset(
            ...
            content_hash=physical_hash,
            storage_key=safe_svg_filename,
            preview_key=safe_svg_filename,
        ...
        try:
            storage.delete(safe_svg_filename)
        except Exception:
            pass
        AssetStore.delete_asset_record(blob_id)
        raise db_err
    ...
    return {
        ...
        "thumbnail_url": f"/api/images/{image_id}/svg",
        "svg_url": f"/api/images/{image_id}/svg",
        "json_url": f"/api/images/{image_id}",
        "width": req.width,
        "height": req.height,
...
def get_image(image_id: str):
    if not re.fullmatch(r"[0-9a-f]{24,64}", image_id):
        return None
    return read_generated_image(image_id)


def list_recent_images(*, owner_id: Optional[str], is_admin: bool,
                       is_authenticated: bool, limit: int):
    """Return bounded, lightweight DTOs for the recent-image gallery."""
    docs = list_recent_generated_image_records(
        owner_id=owner_id,
        is_admin=is_admin,
    ...
    for doc in docs:
        ...
        items.append({
            ...
            "access_scope": doc.get("access_scope", "public" if not doc.get("owner_id") else "private"),
...
async def generate_image_plan(prompt: str, owner_id: Optional[str] = None, **kwargs) -> Dict[str, Any]:
    """Tạo hoặc lấy kết quả sinh ảnh với cơ chế deduplication và phân quyền."""
    req = ImageRequest(prompt=prompt, **kwargs)
    res = create_image(req, owner_id=owner_id)
    if "url" not in res:
        res["url"] = res.get("image_url") or res.get("svg_url") or f"/api/images/{res['image_id']}"
    return res
