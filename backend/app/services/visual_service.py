...
from backend.app.data_access.operations.admission_visual_operations import (
    persist_admission_visual,
    read_admission_visual,
    search_admission_visuals,
)

_in_memory_visual_cache: Dict[str, Dict[str, Any]] = {}

_HEAVY_VISUAL_FIELDS = {"image_base64", "image_data", "binary", "blob"}


def _compact_visual_doc(doc: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    """Loại bỏ binary/Base64 khỏi JSON domain trước khi cache hoặc trả qua API."""
    if not doc:
        return None
    compact = copy.deepcopy(doc)
    for field in _HEAVY_VISUAL_FIELDS:
...
def save_visual_document(doc: Dict[str, Any]) -> bool:
    ...
    doc = _compact_visual_doc(doc) or {}
    _in_memory_visual_cache[visual_id] = copy.deepcopy(doc)
    
    try:
        return persist_admission_visual(doc)
    except Exception as exc:
        print(f"[WARN] Failed to save visual through operation gateway: {type(exc).__name__}")
        return False
...
def get_visual_by_id(visual_id: str) -> Optional[Dict[str, Any]]:
    ...
    if visual_id in _in_memory_visual_cache:
        return copy.deepcopy(_in_memory_visual_cache[visual_id])
    
    try:
        doc = read_admission_visual(visual_id)
        if doc:
...
def list_all_visuals(category: str = "", search: str = "") -> list:
    ...
    try:
        docs = search_admission_visuals(category=category or "all", search=search, limit=100)
        if docs:
            return [_compact_visual_doc(d) or {} for d in docs]
    except Exception as exc:
        print(f"[WARN] Failed to query visuals through operation gateway: {type(exc).__name__}")

    bundle_path = settings.ADMISSION_VISUALS_PATH
    if bundle_path.exists():
        try:
            with open(bundle_path, "r", encoding="utf-8") as f:
                ...
                return [_compact_visual_doc(d) or {} for d in data]
        except Exception:
            pass
    return [copy.deepcopy(v) for v in _in_memory_visual_cache.values()]
...
def export_visual_to_excel(data: Dict[str, Any]) -> io.BytesIO:
    ...
    wb.properties.created = fixed_workbook_time
    wb.properties.modified = fixed_workbook_time
    wb.properties.creator = "HUIT AI Assistant"
    wb.properties.lastModifiedBy = "HUIT AI Assistant"
    ws = wb.active
    ...
    ws["A3"] = f"{subtitle} • Xuất từ HUIT AI Assistant"
    ws["A3"].font = sub_font
    ...
    # XLSX là ZIP; thư viện ZIP thường chèn timestamp hiện tại vào từng entry.
    # Chuẩn hóa thứ tự và timestamp để cùng manifest sinh đúng cùng bytes/checksum.
    canonical_buf = io.BytesIO()
    raw_buf.seek(0)
    with zipfile.ZipFile(raw_buf, "r") as source, zipfile.ZipFile(
        canonical_buf,
        "w",
        ...
        for name in sorted(source.namelist()):
            ...
            entry_bytes = source.read(name)
            if name == "docProps/core.xml":
                entry_bytes = re.sub(
                    rb"<dcterms:modified[^>]*>.*?</dcterms:modified>",
                    rb'<dcterms:modified xmlns:dcterms="http://purl.org/dc/terms/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:type="dcterms:W3CDTF">2000-01-01T00:00:00Z</dcterms:modified>',
                    entry_bytes,
                )
            target.writestr(target_info, entry_bytes, compress_type=zipfile.ZIP_DEFLATED, compresslevel=9)
    canonical_buf.seek(0)
    return canonical_buf

def resolve_visual_for_query(intent: str, question: str, docs: list = None) -> Optional[Dict[str, Any]]:
        ...
        if docs:
            for d in docs:
                ...
                if code:
                    ...
                    break
        matched = find_visual_by_context(intent, question, major_code=primary_code)
        if matched:
            ...
            return {
                ...
                "preview_url": f"/api/admission-visuals/{v_id}/render",
                "manifest_url": f"/api/admission-visuals/{v_id}",
                "available_formats": ["svg", "png", "xlsx"],
